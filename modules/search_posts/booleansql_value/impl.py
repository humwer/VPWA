from utils import *

'''
Boolean-based SQLi при поиске постов в параметре value
'''


def search_posts(column, value) -> tuple:
    data = []

    try:
        conn, cursor = settings.connect_to_db()
        if ";" in value or "union" in value.lower():
            raise Exception("[?] Крутят скулю в поиске и пытаются сделать stacked/union")
        if '"' in column or ";" in column:
            raise Exception("[?] Крутят скулю в фильтре")
        query = f"SELECT * FROM posts WHERE \"{column}\" like '%{value}%';"
        posts = cursor.execute(query).fetchall()
        cursor.close()
        for post in posts:
            data.append({'id': post[0], 'author': post[1], 'title': post[2],
                         'tags': post[3].split(','), 'path': post[4], 'visible': post[5]})
        return sorted(data, key=lambda x: x['id'], reverse=True), ""

    except Exception as err:
        print(f"[?] {err}")
        return data